...
import getAssociationsAPI from '@/app/api/getAssosiationAPI'
...
export default function MusicAssociations(){

    const [authToken , setAuthToken] = useState(null)
    const [assosiation,setAssociations] = useState([])
    ...
    useEffect(() => {
        try{
            async function fetch() {
                try{
                    let result = await getAssociationsAPI()

                    await setAssociations(result)
    ...
    const renderElement = () => {
        ...
        for (let i = 0 ; i < assosiation.length ; i+=4){
            renderedElements.push(
                            ...
                            <div className='group-div' id={i}>
                                    {
                                        (authToken)?
                                        (
                                            renderButton(assosiation[i].id)
                                        ):
                                        (<button className='follow-btn' onClick={() => goToLogin()}><FaPlus className='icon'/>follow</button>)
                                    }  
                                    
                                    <img className='background' src={`${IMG_KEY}${assosiation[i].background_image}`} alt={`${assosiation[i].name} background`} onError={(e) => handleImageError(e)}></img>    
                                    <div className='image-des-container'>
                                        <img src={`${IMG_KEY}${assosiation[i].profile_image}`} alt={`${assosiation[i].name} profile`} onError={(e) => handleImageError(e)}/>
                                        <div className='des'>
                                            <h1>{assosiation[i].name}</h1>
                            ...
                            <div className='group-div' id={i+1}>  
                                    {
                                        (authToken)?
                                        (
                                            renderButton(assosiation[i+1].id)
                                        ):
                                        (<button className='follow-btn'><FaPlus className='icon' onClick={goToLogin}/>follow</button>)
                                    }  
                                    <img className='background' src={`${IMG_KEY}${assosiation[i+1].background_image}`} alt={`${assosiation[i+1].name} background`} onError={(e) => handleImageError(e)}></img>    
                                    <div className='image-des-container'>
                                        <img src={`${IMG_KEY}${assosiation[i+1].profile_image}`} alt={`${assosiation[i+1].name} profile`} onError={(e) => handleImageError(e)}/>
                                        <div className='des'>
                            ...
                            <div className='group-div' id={i+2}>  
                                    {
                                        (authToken)?
                                        (
                                            renderButton(assosiation[i+2].id)
                                        ):
                                        (<button className='follow-btn'><FaPlus className='icon' onClick={goToLogin}/>follow</button>)
                                    }  
                                    <img className='background' src={`${IMG_KEY}${assosiation[i+2].background_image}`} alt={`${assosiation[i+2].name} background`} onError={(e) => handleImageError(e)}></img>    
                                    <div className='image-des-container'>
                                        <img src={`${IMG_KEY}${assosiation[i+2].profile_image}`} alt={`${assosiation[i+2].name} profile`} onError={(e) => handleImageError(e)}/>
                                        <div className='des'>
                                            <h1>{assosiation[i+2].name}</h1>
                            ...
                            <div className='group-div' id={i+3}>
                                    {
                                        (authToken)?
                                        (
                                            renderButton(assosiation[i+3].id)
                                        ):
                                        (<button className='follow-btn'><FaPlus className='icon' onClick={goToLogin}/>follow</button>)
                                    }    
                                    <img className='background' src={`${IMG_KEY}${assosiation[i+3].background_image}`} alt={`${assosiation[i+3].name} background`} onError={(e) => handleImageError(e)}></img>    
                                    <div className='image-des-container'>
                                        <img src={`${IMG_KEY}${assosiation[i+3].profile_image}`} alt={`${assosiation[i+3].name} profile`} onError={(e) => handleImageError(e)}/>
                                        <div className='des'>
                                            <h1>{assosiation[i+3].name}</h1>
